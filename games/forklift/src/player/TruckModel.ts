...
export class TruckModel {
  ...
  constructor(f: Factory, root: TransformNode, forkRoot: TransformNode, deferred = false) {
    if (!deferred) finishConstruction(this.build(f, root, forkRoot));
  }
  static async create(f: Factory, root: TransformNode, forkRoot: TransformNode) {
    const model = new TruckModel(f, root, forkRoot, true);
    await constructScene(model.build(f, root, forkRoot), { isCancelled: () => f.scene.isDisposed });
    return model;
  }
  private *build(f: Factory, root: TransformNode, forkRoot: TransformNode) {
    const scene = f.scene;
    ...
    yield;
    for (let i = 0; i < 7; i++) { yield; box('horizontal cooling louvre', [.78, .016, .027], [0, .065 + i * .038, -1.416], steel, .007); }
    for (const side of [-1, 1]) {
      ...
      yield;
      for (let j = 0; j < 7; j++) { yield; box('anti slip step ribs', [.2, .015, .024], [side * .92, -.17, -.27 + j * .078], chrome, .006); }
      ...
      yield;
      for (let j = 0; j < 5; j++) { yield; box('side cooling slot', [.012, .018, .23], [side * .902, .04 + j * .044, -.65], dark, .004); }
    }
    ...
    for (const x of [-.88, .88]) {
      yield; for (const z of [-.87, .8]) {
        ...
        for (const side of [-1, 1]) {
          yield;
          const bead = torus('moulded sidewall bead', .55, .012, [side * .171, 0, 0], tread, wheel);
          yield; bead.rotation.z = Math.PI / 2;
          const rim = cylinder('dished alloy rim', .425, .045, [side * .145, 0, 0], this.rims, wheel);
          yield; rim.rotation.z = Math.PI / 2;
          const lip = torus('wheel rim rolled lip', .414, .028, [side * .17, 0, 0], this.rims, wheel);
          yield; lip.rotation.z = Math.PI / 2;
          const hub = cylinder('axle dust cap', .16, .065, [side * .18, 0, 0], steel, wheel);
          yield; hub.rotation.z = Math.PI / 2;
          for (let i = 0; i < 6; i++) {
            yield;
            const a = i * Math.PI / 3;
            const bolt = cylinder('hex wheel nut', .038, .025, [side * .18, Math.sin(a) * .139, Math.cos(a) * .139], chrome, wheel, 6);
            yield; bolt.rotation.z = Math.PI / 2;
          }
        }
        for (let i = 0; i < 36; i++) {
          yield; for (const side of [-1, 1]) {
            yield;
    ...
    yield;
    for (let i = 0; i < 9; i++) { yield; box('floor mat rib', [.92, .016, .018], [0, .089, -.16 + i * .082], rubber, .006); }
    box('seat suspension pedestal', [.43, .22, .41], [0, .37, -.33], steel, .04);
    yield;
    for (let i = 0; i < 4; i++) { yield; box('seat suspension bellows', [.49, .04, .45], [0, .32 + i * .045, -.33], rubber, .018); }
    box('sculpted seat cushion', [.62, .18, .62], [0, .56, -.24], seat, .085);
    yield;
    for (const x of [-.29, .29]) { yield; box('seat side bolster', [.115, .19, .54], [x, .61, -.24], seat, .055); }
    const back = box('contoured seat back', [.62, .62, .18], [0, .89, -.51], seat, .085);
    yield; back.rotation.x = -.12;
    for (const x of [-.275, .275]) {
      yield; const bolster = box('backrest bolster', [.13, .48, .19], [x, .88, -.445], seat, .06);
      yield; bolster.rotation.x = -.12;
    }
    box('lumbar cushion', [.38, .15, .065], [0, .77, -.384], rubber, .03);
    yield;
    box('seat belt buckle', [.065, .1, .07], [.37, .58, -.32], red, .025);
    yield;
    for (const x of [-.32, .32]) { yield; tube('seat arm rest', [[x, .6, -.44], [x, .82, -.4], [x, .84, -.04]], .035, seat); }
    ...
    for (const z of [-.79, .86]) { yield; box('overhead cross member', [1.7, .14, .15], [0, 2.16, z], this.paint, .055); }
    for (let i = 0; i < 6; i++) { yield; box('overhead safety slat', [.065, .065, 1.56], [-.58 + i * .232, 2.17, .035], this.frame, .018); }
    box('curved weather canopy', [1.65, .07, 1.78], [0, 2.235, .025], this.paint, .034, this.canopy);
    yield;
    for (const x of [-.8, .8]) { yield; box('canopy rain gutter', [.035, .04, 1.75], [x, 2.25, .025], this.frame, .014, this.canopy); }
    ...
    yield;
    for (let i = 0; i < 3; i++) { yield; torus('beacon lens ring', .19, .007, [0, .035 + i * .03, 0], amber, beacon); }
    ...
    const display = box('inset instrument display', [.27, .018, .13], [-.21, .929, .59], screen, .008);
    yield; display.rotation.x = -.16;
    for (let i = 0; i < 4; i++) { yield; box('dashboard indicator', [.025, .017, .028], [-.3 + i * .05, .946, .58], this.lenses, .006); }
    ...
    for (const x of [-.25, .2]) {
      yield; const pedal = box('ribbed driving pedal', [.17, .06, .23], [x, .15, .57], rubber, .02);
      yield; pedal.rotation.x = -.35;
    }
    ...
    for (const x of [-.68, .68]) {
      ...
      yield;
      for (const z of [-.24, -.04]) { yield; box('mast channel flange', [.18, 2.9, .045], [x, 1.35, z], steel, .012, this.mast); }
      box('sliding inner rail', [.09, 2.55, .1], [x * .82, 1.22, -.105], chrome, .018, this.innerMast);
      yield;
    }
    for (const y of [.1, 2.74]) { yield; box('mast cross tie', [1.42, .12, .2], [0, y, -.14], steel, .025, this.mast); }
    ...
    for (const x of [-.43, .43]) {
      ...
      yield;
      const rod = cylinder('telescoping polished piston', .073, 1, [x, 1.12, -.16], chrome, this.mast);
      yield; this.pistons.push(rod);
      const pulley = cylinder('lift chain sheave', .18, .1, [x, 2.47, -.12], steel, this.innerMast);
      yield; pulley.rotation.z = Math.PI / 2;
      tube('hydraulic supply hose', [[x, .0, -.23], [x + .12, .22, -.31], [x + .12, .62, -.31], [x, .9, -.22]], .018, rubber, this.mast);
      yield;
      for (let i = 0; i < 31; i++) { yield; box('roller chain link', [.048, .035, .03], [x, .13 + i * .074, .015], forkSteel, .008, this.innerMast); }
    }
    for (const x of [-.62, .62]) { yield; place(forgedFork('forged tapered tine', scene), [x, 0, 0], forkSteel, forkRoot); }
    for (const y of [.22, 1.1]) { yield; box('carriage cross rail', [1.58, .12, .14], [0, y, -.04], steel, .025, forkRoot); }
    for (const x of [-.74, .74]) { yield; box('backrest outer frame', [.08, 1.02, .1], [x, .61, -.04], steel, .023, forkRoot); }
    for (let i = 0; i < 7; i++) { yield; box('load backrest grille', [.025, .78, .035], [-.57 + i * .19, .66, -.04], steel, .01, forkRoot); }
    for (const x of [-.62, .62]) { yield; box('fork locking pin', [.1, .075, .1], [x, 1.0, .04], chrome, .02, forkRoot); }
    ...
    yield;
    for (let i = 0; i < 8; i++) { yield; box('utility LED lens', [.11, .075, .02], [-.52 + i * .15, 2.3, .805], this.lenses, .009, this.utility); }
    ...
    yield;
    for (const y of [.12, .32]) { yield; torus('extinguisher mounting strap', .18, .018, [0, y, 0], steel, extinguisher); }
    ...
    for (const [parent, byMat] of batches) {
      yield; for (const [material, group] of byMat) {
        yield;
        if (group.length < 2) continue;
        parent.computeWorldMatrix(true);
        for (const mesh of group) { yield; mesh.computeWorldMatrix(true); }
