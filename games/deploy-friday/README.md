...
- Protect DB through seven 75-second waves. Each wave has 60 seconds of spawning and 15 seconds of quieter traffic. Final stragglers must resolve before victory.
...
- A win unlocks a new endless shift. Scores and the unlock are stored locally with a session-only fallback if browser storage is unavailable. Nothing is uploaded.
...
Tests cover deterministic replay, costs/refunds, route safety, all request/service behaviors, ability timing, exact rollback, final-wave draining and a full budget-funded Friday. Balance was also smoke-tested over seeds 1, 7, 42, 123, 1655, 2026 and 99999 using a fixed build/upgrade strategy.
