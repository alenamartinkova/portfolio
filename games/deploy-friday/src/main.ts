...
function update() {
  ...
  $('wave').textContent = training ? `${c.lesson} ${training.index + 1} / ${LESSONS.length}` : `${t.wave} ${s.wave} / ${s.endless ? '∞' : level.waves}`; $('clock').textContent = clockLabel();
  $('time-progress').style.width = `${training ? (training.index + Number(training.status === 'done')) / LESSONS.length * 100 : Math.min(100, s.tick / (level.waves * level.waveSeconds * HZ) * 100)}%`;
  ...
  $('mission-number').textContent = s.endless ? '∞' : String(s.mission).padStart(2, '0');
  $('mission-title').textContent = s.endless ? t.endless : c.levels[s.mission].title;
  $('mission-threat').textContent = c.levels[s.mission].threat;
  $('mission-briefing').hidden = !!training || s.phase !== 'planning';
  const briefing = `${c.briefing}: ${c.victory} ${c.levels[s.mission].tip}`;
  if ($('mission-briefing').textContent !== briefing) $('mission-briefing').textContent = briefing;
  $('live').textContent = paused && s.phase === 'running' ? t.paused : s.phase === 'planning' ? t.ready : s.phase === 'lost' ? 'INCIDENT' : t.live;
  $('pause').textContent = paused ? t.resume : t.pause; $<HTMLButtonElement>('pause').disabled = s.phase !== 'running' || !!training && training.status !== 'watch';
