...
export class FollowCamera {
    ...
    constructor(private scene: Scene, spawn: Vector3) { this.target.copyFrom(spawn); this.camera = new FreeCamera('follow camera', spawn.add(new Vector3(4, 7, -10)), scene); this.camera.fov = .88; this.camera.minZ = .08; this.camera.maxZ = 160; }
    update(dt: number, p: Vector3, input: Input, overview = false) {
        ...
        input.lookX = 0;
        input.lookY = 0;
        this.target = Vector3.Lerp(this.target, p.add(new Vector3(0, .45, 0)), 1 - Math.exp(-12 * dt));
        const dist = overview ? 14 : 7.6;
        ...
        const hit = this.scene.pickWithRay(ray, m => Boolean(m.metadata?.solid) && !m.metadata?.dynamic);
    ...
    snap(p: Vector3) { this.target.copyFrom(p); this.camera.position.copyFrom(p.add(new Vector3(3, 5, -7))); }
}
