...
export class FollowCamera {
    ...
    private aim = Vector3.Zero();
    private offset = Vector3.Zero();
    private desired = Vector3.Zero();
    private ray = new Ray(this.target, this.offset);
    private pickStatic = createCachedRaycast((ray: Ray) => this.scene.pickWithRay(ray, m => Boolean(m.metadata?.solid) && !m.metadata?.dynamic));
    ...
    update(dt: number, p: Vector3, input: Input, overview = false) {
        ...
        input.lookX = 0;
        input.lookY = 0;
        this.aim.copyFrom(overview && this.overviewTarget ? this.overviewTarget : p);
        if (!(overview && this.overviewTarget)) this.aim.y += .45;
        Vector3.LerpToRef(this.target, this.aim, 1 - Math.exp(-12 * dt), this.target);
        ...
        this.ray.length = dist;
        const hit = overview ? null : this.pickStatic(this.ray);
        const distance = hit?.hit ? Math.max(.7, hit.distance - .25) : dist;
        this.offset.scaleToRef(distance, this.desired); this.desired.addInPlace(this.target);
        this.impulse *= Math.exp(-14 * dt);
        this.desired.y += this.impulse;
        Vector3.LerpToRef(this.camera.position, this.desired, 1 - Math.exp(-10 * dt), this.camera.position);
