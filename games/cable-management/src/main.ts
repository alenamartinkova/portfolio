...
function movePointer(event: PointerEvent) {
  ...
  if (grip) {
    ...
    scene.preview(drawer, current(), hover ?? undefined);
  }
}
...
app.addEventListener('pointerdown', startPointer);
  ...
  if (!document.hidden) {
    if (spec.mode === 'untangle' && !complete) {
      ...
      if (tick++ % 6 === 0) {
        ...
        el('crossing-markers').innerHTML = points
          .slice(0, 28)
          .map((p) => `<i style="left:${p.x}px;top:${p.y}px"></i>`)
          .join('');
        updateStatus();
      }
      calm =
